...
`dsidle_run_ycsb_experiment.sh` 固定面向 4 VM 实验。它校验 workload 矩阵、
物化配置和 `run_meta.json`，以本仓固定的 YCSB-cpp 子模块生成 trace，并调用
VM 回放编排。镜像只构建一次；二进制和 ivshmem 模块在宿主的 Jammy 根文件系统
中构建一次，然后由 rsync 分发到四台 VM。load 与 run 都是每 VM 多 worker 并发
回放，load 不存在串行降级路径。
...
./dsidle_run_ycsb_experiment.sh \
  --rounds 10 --record-count 100000 --operation-count 100000 \
