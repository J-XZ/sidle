...
for index in range(int(vm['count'])):
    ...
    if dry:
        print(f'DRY_RUN verify pid={pidfile} qemu_cmdline=ivshmem:{shared["path"]} taskset={",".join(map(str, sorted(wanted_cores)))} ssh={ssh_user}@127.0.0.1:{port} device={shared["device_path"]} numa_nodes={",".join(map(str, nodes))}')
        continue
    ...
    if 'qemu-system-x86_64' not in cmdline or str(shared['path']) not in cmdline or 'ivshmem-plain' not in cmdline:
    ...
    for line in Path(f'/proc/{pid}/numa_maps').read_text(errors='replace').splitlines():
        if str(shared['path']) not in line: continue
        mapped_nodes.update(int(item.split('=')[0][1:]) for item in line.split() if re.fullmatch(r'N\d+=\d+', item))
    if not mapped_nodes: raise SystemExit(f'pid {pid} has no numa_maps entry for {shared["path"]}')
    if not mapped_nodes <= set(map(int, nodes)):
