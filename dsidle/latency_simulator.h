...
inline void RecordSwccRead(const void *address, uint64_t bytes) {
  if (InstrumentationEnabledFast())
    GlobalLatencySimulator().RecordRange(PoolKind::kSwcc, AccessKind::kRead,
                                         address, bytes);
}
inline void RecordSwccWrite(const void *address, uint64_t bytes) {
  if (InstrumentationEnabledFast())
    GlobalLatencySimulator().RecordRange(PoolKind::kSwcc, AccessKind::kWrite,
                                         address, bytes);
}
inline void RecordSwccFlush(const void *address, uint64_t bytes) {
  if (InstrumentationEnabledFast())
...
inline void RecordHwccAtomicLoad(const void *address) {
  if (InstrumentationEnabledFast())
    GlobalLatencySimulator().RecordLine(PoolKind::kHwcc,
                                        AccessKind::kAtomicLoad, address);
}
inline void RecordHwccAtomicStore(const void *address) {
  if (InstrumentationEnabledFast())
    GlobalLatencySimulator().RecordLine(PoolKind::kHwcc,
                                        AccessKind::kAtomicStore, address);
}
inline void RecordHwccAtomicRmw(const void *address) {
  if (InstrumentationEnabledFast())
    GlobalLatencySimulator().RecordLine(PoolKind::kHwcc,
                                        AccessKind::kAtomicRmw, address);
}
