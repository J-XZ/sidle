...
usage() {
  echo "usage: $0 [--config PATH] [--image PATH] [--pool-tool PATH] [--dry-run] [--allow-overlapping-numa] [--apply-host-tuning|--no-host-tuning] --execute" >&2
}
while (($#)); do
  case "$1" in
...
((!(no_host_tuning && apply_host_tuning))) || {
  echo "--apply-host-tuning and --no-host-tuning are mutually exclusive" >&2
  exit 2
}
...
((apply_host_tuning)) && cmd+=(--apply-host-tuning)
